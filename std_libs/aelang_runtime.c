...
#include <errno.h>
#include <limits.h>
#include "aelang_runtime.h"

// =============================================================================
// ÆLANG RUNTIME LIBRARY IMPLEMENTATION
...
// Global storage for different return types from universal read
static char read_string_buffer[1024];
static double read_float_value;
static long long read_int_value;

// Truly universal read function that handles all types
// Returns different values based on input format detection
int read() {
    char input_buffer[1024];
    ...
    // Try as integer
    errno = 0;
    long long_val = strtoll(start, &endptr, 10);
    if (*endptr == '\0' && errno == 0) {
        // Valid integer - store for different precisions
        read_int_value = long_val;
        if (long_val >= INT_MIN && long_val <= INT_MAX) {
            return (int)long_val;
        } else {
            // Large integer, return truncated
            return (int)long_val;
        }
    }
    ...
    errno = 0;  
    double double_val = strtod(start, &endptr);
    if (*endptr == '\0' && errno == 0 && !isinf(double_val) && !isnan(double_val)) {
        // Valid float - store both float and int representation
        read_float_value = double_val;
        
        // Return as int bits for assembly to convert back to float
        union { float f; int i; } converter;
        converter.f = (float)double_val;
        return converter.i;
    }
    
    // Default to string - copy to global buffer and return pointer as int
    strncpy(read_string_buffer, start, sizeof(read_string_buffer) - 1);
    read_string_buffer[sizeof(read_string_buffer) - 1] = '\0';
    
    // Return pointer as integer (will be converted back by assembly/compiler)
    return (int)(uintptr_t)read_string_buffer;
}
