...
// Basic print function for strings (adds newline)
void print_simple(const char *str) {
    printf("%s\n", str);
    fflush(stdout);
}

// Printf-style function with format specifiers (no extra newline)
...
short read_i16() {
    ...
    if (fgets(buffer, sizeof(buffer), stdin)) {
        ...
        if (*endptr == '\0') { // Ensure the whole string was a valid number
            if (value >= -32768 && value <= 32767) {
                return (short)value;
            } else {
                printf("Error: i16 value out of range. Please enter a value between -32768 and 32767.\\n");
                return 0;
            }
        }
    }
    printf("Error: Invalid i16 input. Expected an integer.\\n");
    return 0;
}
