...
static LLVMTypeRef semantic_type_to_llvm(LLVMBackend* backend, SemanticType type) {
    switch (type) {
        ...
        case TYPE_STRING: return LLVMPointerType(LLVMInt8TypeInContext(backend->context), 0);
...
int llvm_generate_from_ir(LLVMBackend* backend, IRProgram* program) {
    if (!backend || !program) {
...
// Emit assembly file
int llvm_emit_assembly(LLVMBackend* backend, const char* filename, const char* target_triple) {
    if (!backend || !filename) return 0;
