...
static IROperand *generate_expression_ir_with_type(AnnotatedASTNode *node, IRContext *ctx, SemanticType expected_type) {
    ...
    switch (ast_node->type) {
        case AST_FUNC_CALL: {
            ...
            printf("[IR Debug] Function call: %s with %zu arguments\n", ast_node->as.func_call.name, ast_node->as.func_call.arg_count);
            
            for (size_t i = 0; i < ast_node->as.func_call.arg_count; i++) {
                printf("[IR Debug] Processing argument %zu\n", i);
                args[i] = generate_expression_ir(node->children[i], ctx);
                if (args[i]) {
                    printf("[IR Debug] Argument %zu: type=%d\n", i, args[i]->type);
                } else {
                    printf("[IR Debug] Argument %zu: NULL\n", i);
                }
            }
...
static IROperand *generate_expression_ir(AnnotatedASTNode *node, IRContext *ctx) {
    ...
    switch (ast_node->type) {
        ...
        case AST_BIN_OP: {
            ...
            else if (strcmp(op, "&&") == 0) opcode = IR_LOGICAL_AND;
            else if (strcmp(op, "||") == 0) opcode = IR_LOGICAL_OR;
            else if (strcmp(op, "&") == 0) opcode = IR_BITWISE_AND;
            else if (strcmp(op, "|") == 0) opcode = IR_BITWISE_OR;
            else if (strcmp(op, "^") == 0) opcode = IR_BITWISE_XOR;
...
const char *opcode_to_string(IROpcode opcode) {
    switch (opcode) {
        ...
        case IR_BITWISE_AND: return "BITWISE_AND";
        case IR_BITWISE_OR: return "BITWISE_OR";
        case IR_BITWISE_XOR: return "BITWISE_XOR";
        case IR_BITWISE_NOT: return "BITWISE_NOT";
