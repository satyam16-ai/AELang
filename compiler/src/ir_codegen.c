...
static int current_arg_count = 0; // Track arguments for current function call

// Parameter collection for correct calling convention
static IROperand *param_buffer[100];  // Buffer to collect parameters
static int param_buffer_count = 0;    // Number of parameters in buffer
...
// Get argument register for System V ABI (x86-64)
...
void emit_ir_instruction(IRInstruction *instr, FILE *out, CompilationConfig *config) {
    ...
    } else if (instr->opcode >= IR_AND && instr->opcode <= IR_NEG) {
